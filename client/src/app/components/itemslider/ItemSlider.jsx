...
const ItemSlider = ({ title, children }) => {
  ...
  return (
    <Container fluid className='my-3 py-3 item-slider-container'>
      ...
      <div className='item-slider'>
        <div onClick={() => slide(-400)} className={`left-arrow-left ${(scrollX < 1) ? 'is-disabled-hide' : ''}`}>
          <MdArrowBackIos size="70px" />
        </div>
        <div ref={scrl} onScroll={scrollCheck} className="item-container">
          {children}
        </div>
        <div className={`right-arrow-right ${(!scrollEnd) ? '' : 'is-disabled-hide'}`} onClick={() => slide(+400)}>
